...
Data from Toby Marthews, [r-sig-mixed-models mailing list](http://tolstoy.newcastle.edu.au/R/e13/help/11/01/1364.html): you can find it [here](https://github.com/bbolker/krakow/tree/master/data).
...
ggplot(dataf,aes(x=mnth,y=stmass))+
  ...
  facet_wrap(~roostsitu,nrow=1)        ## 1 row of panels by roost
```
...
What does it look like if you use `colour=subject` ?
...
it reports the variances and standard deviations
...
ggplot(augment(lmer2),
       aes(sample=.resid/sd(.resid)))+  ## scale to variance=1
        stat_qq(aes(colour=roostsitu))+
    geom_abline(intercept=0,slope=1)
```

With `colour` specified this computes Q-Q statistics for each group separately - there is no super-easy way to compute the Q-Q statistics for the whole data set and then draw the colours per group.
...
```{r coefplot}
dwplot(tidy(lmer2,effects="fixed"))+geom_vline(xintercept=0,lty=2)
```

`dwplot` automatically drops the intercept. `dwplot(lmer2)` should just work, but doesn't at present ...
...
a2 <- anova(lmer2B)
fvals <- a2[,"F value"]
numdf <- a2[,"NumDF"]  ## might differ with devel version of lmerTest?
dendf <- 36
...
Unfortunately computing confidence intervals for the predictions is a little tricky: again, there is some code on the [GLMM faq](http://bbolker.github.io/mixedmodels-misc/glmmFAQ) for this (also see below under `MCMCglmm`).
...
* updated, Jagielloninan workshop, Apr 2018
